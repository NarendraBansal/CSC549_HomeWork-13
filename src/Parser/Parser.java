...
import ParserTree.RememberStatementTree;
import ParserTree.StatementTree;
import ParserTree.SymbolTree;
...
public class Parser 
{
	...
	public static void parse(String filename)
	{
		try
		{
			...
			{
				Parser.parseToken(theProgramLines[i]);
	...
	public static void parseToken(String line) {
		String tokens[]=line.split(" ");
		ArrayList li= new ArrayList();
		Int_LiteralExpression ile=null;
		RememberStatementTree rst=null;
		System.out.println("Program Tree");
		for(String token:tokens) {
			
			
			if(LanguageCore.isStatementWord(token)) {
				if(token.equals("remember")) {
					rst=new RememberStatementTree();
				}
				...
				if(token.equals("=")) {
					...
					ile= new Int_LiteralExpression(new Integer(expression));
				...
				if(exp==false) {
					SymbolTree syTr = new SymbolTree(token);
					li.add(syTr);
				}
				
				}
				rst.theComponents=li;
			}
